<script>
import {UsersApiService} from "@/services/service/users-api.service";
import {User} from "@/services/model/users.entity.js";
...
export default {
    ...
    async mounted() {
        const apiUserService = new UsersApiService();
        try {
            const response = await apiUserService.getAll();
            this.user = response.data.map(userData => User.fromApiResponse(userData));
